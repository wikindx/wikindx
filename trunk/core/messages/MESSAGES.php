...
/// heading_exportCoins COinS is a bibliographic format used by, for instance, Zotero. Don't translate 'COinS'
				...
				"importEndnote" => dgettext($domain, "Import EndNote XML Bibliography"),
				"importPubMed" => dgettext($domain, "Import PubMed"),
				"rtfExport" => dgettext($domain, "Export Rich Text Format"),
				"bibtexExport" => dgettext($domain, "Export BibTeX"),
				"htmlExport" => dgettext($domain, "Export HTML"),
				"risExport" => dgettext($domain, "Export RIS"),
				"endnoteExport" => dgettext($domain, "Export EndNote"),
				"ideaExport" => dgettext($domain, "Export Ideas"),
				"listFiles" => dgettext($domain, "Exported Files"),
				"bibutils" => dgettext($domain, "Bibutils Conversion"),
			),
...
/// config_pagingTagCloud Tag clouds are the display of creators, keywords, collections etc. found under the resources menu.
				...
				"sessionAuthMaxlifetime" => dgettext($domain, "Max life time of a user session"),
				"sessionNotAuthMaxlifetime" => dgettext($domain, "Max life time of an anonymous session"),
...
/// config_authGate If checked, user must click on OK after login in order to proceed. Used for situations such as notification about privacy policies such as that mandated by the EU's GDPR
				...
				"siteMapDisallow" => dgettext($domain, "Disable SiteMap"),
				"browserTabID" => dgettext($domain, "Enable BrowserTabID"),
				"rssDisallow" => dgettext($domain, "Disable syndication feeds (RSS/Atom)"),
				"rssDisplayEditedResources" => dgettext($domain, "Display also the last edited resources"),
...
/// components_downloadSignature is printed as one message with corruptDownload and corruptHash messages
				...
				"compressionFormatError" => dgettext($domain, "The component manager only supports Zip format. Other compression formats are provided for manual installations only."),
...
/// hint_pagingInfoOf Follows hint_pagingInfo: '... of 345'
				...
				"sessionAuthMaxlifetime" => dgettext($domain, "Max. lifetime of sessions which have been authenticated at least once. This is for a registered user – the session is kept alive with activity within this period (measured back from the latest WIKINDX access), otherwise, the session is deleted."),
				"sessionNotAuthMaxlifetime" => dgettext($domain, "Max. lifetime of sessions which have never been authenticated. This is for a read-only user – the session is kept alive with activity within this period (measured back from the latest WIKINDX access), otherwise, the session is deleted."),
				...
				"siteMapDisallow" => dgettext($domain, "Sitemaps are essential for a good indexing of a website by search engines. WIKINDX sitemap indexes public resources and news. For details, see https://www.sitemaps.org/."),
				...
				"rssDisallow" => dgettext($domain, "If checked, disallow the access to syndication feeds (RSS/Atom) and hide them."),
				"rssDisplayEditedResources" => dgettext($domain, "Check to display the last added AND edited resources. Uncheck to display the last added resources."),
				"rssLimit" => dgettext($domain, "Number of recent items to send to the feed. Default is 50. Advice: use a value that can cover a day of resource modification."),
				...
				"gsDisallow" => dgettext($domain, "Disallow Google Scholar to index resources"),
...
/// hint_storeRawBibtex Do not translate '@string'
				...
				"bibUtils" => dgettext($domain, "The Bibutils plugin uses C binaries written by Chris Putnam at Bibutils. It allows for the format conversion of many different types of bibliographies so that there is usually a way to import your bibliography into WIKINDX. Most conversions use the MODS XML intermediate format. Enter here the path to bibUtils (e.g. '/usr/bin/' for *NIX or 'D:/wamp/www/wikindx/bibutils/' for windows). If this is empty, the plugin's export function will assume *NIX and look by default in \$PATH. Needs the trailing '/'."),
			),
...
/// menu_res Resources menu starts here
				...
				"import" => dgettext($domain, "Import..."),
				"bibutils" => dgettext($domain, "Bibutils"),
				"exportbasket" => dgettext($domain, "Export basket..."),
				"exportlist" => dgettext($domain, "Export list..."),
				"importEndnote" => dgettext($domain, "EndNote"),
				"importPubMed" => dgettext($domain, "PubMed"),
				"exportRtf" => dgettext($domain, "RTF"),
				"exportBibtex" => dgettext($domain, "BibTeX"),
				"exportHTML" => dgettext($domain, "HTML"),
				"exportEndNote" => dgettext($domain, "EndNote"),
				"exportRIS" => dgettext($domain, "RIS"),
				"exportList" => dgettext($domain, "Last List"),
				"listFiles" => dgettext($domain, "Exported Files"),
				"ideaExport" => dgettext($domain, "Export Ideas"),
...
/// menu_pasteBibtex Ordinary user can paste a BibTeX file into a textarea for importing
            	...
            	"exportIdeas" => dgettext($domain, "Export Ideas"),
...
/// importexport_ Messages relating to import/export of bibliographies and ideas
			"importexport" => array(
