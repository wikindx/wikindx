...
class ENDNOTEIMPORT
{
    ...
    public function __construct()
    {
        ...
        GLOBALS::setTplVar('heading', $this->messages->text("heading", "importEndnote"));
    }
    /**
     * init
     *
    ...
    public function process()
    {
        ...
        if (!$parse->version8)
        {
            $this->badInput(HTML\p($this->messages->text("importexport", 'importEndnoteNotv8'), 'error'));
        }
        if (empty($entries))
        {
            $this->badInput(HTML\p($this->messages->text("importexport", 'empty'), 'error'));
        }
    ...
    private function findInvalidFields()
    {
        ...
        { // prompt to map field names
            ...
            else
            {
                ...
                \TEMPSTORAGE\store($this->db, $uuid, ['form' => $pString, 'heading' => $this->messages->text("heading", "importEndnote")]);
    ...
    private function cleanUp($finalInput)
    {
        ...
        if ($finalInput)
        {
            $message = \HTML\p($this->messages->text("importexport", "importEndnoteSuccess"), 'success', 'center');
            \TEMPSTORAGE\store(
                $this->db,
                $uuid,
        ...
        else
        {
            ...
            $pString .= \FORM\formHeader("import_ENDNOTEIMPORT_CORE");
            ...
            $tsArray['heading'] = $this->messages->text("heading", "importEndnote");
