    ...
    private function loadArray()
    {
        $array = [
            ...
            "upgradeDB1" => "
                ...
                <p style='color:red;font-weight:bold'If you have downloaded beta/SVN code, then you are strongly advised not to run it on a production server -- if you wish to test it, either create a new database or make a copy of your existing WIKINDX database and point config.php at it. Wait until all bugs have been dealt with, and the final release version provided, before using the WIKINDX code on a production server.</p> <p>To report bugs etc., go to: https://sourceforge.net/p/wikindx/v5bugs/</strong></p> WIKINDX has detected that this is the first time a new version is being run and that the database requires upgrading. This upgrading is automatic but only the WIKINDX superAdmin may carry it out.<p> You are <strong>strongly advised</strong> to <strong>back up your old database</strong> first. If you do not do this and you experience the memory errors detailed below when upgrading your only copy of the database, then you will have irrevocably corrupted the database: <em>caveat upgrader</em><p>",
