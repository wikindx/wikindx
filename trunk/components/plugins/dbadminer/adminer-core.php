...
error_reporting(6135);$Zc = !preg_match('~^(unsafe_raw)?$~', ini_get("filter.default")); if ($Zc || ini_get("filter.default_flags"))
{
...
}function remove_slashes($vg, $Zc = FALSE)
{
...
as$he => $W)
            {
                ...
                {
                    $vg[$y][stripslashes($he)] = ($Zc ? $W : stripslashes($W));
                }
            }
        }
    }
}function bracket_escape($u, $Pa = FALSE)
{
    static$xi = [':' => ':1',']' => ':2','[' => ':3','"' => ':4'];

    return
strtr($u, ($Pa ? array_flip($xi) : $xi));
}function min_version($dj, $Ge = "", $i = NULL)
{
    global$h;
    if (!$i)
    {
...
}function nl_br($P)
{
    return
str_replace("\n", "<br>", $P);
}function checkbox($B, $Y, $gb, $oe = "", $xf = "", $lb = "", $pe = "")
{
...
}function optionlist($Cf, $kh = NULL, $Vi = FALSE)
{
    $H = "";
    foreach ($Cf
as$he => $W)
...
as$y => $X)
        {
            $H .= '<option' . ($Vi || is_string($y) ? ' value="' . h($y) . '"' : '') . (($Vi || is_string($y) ? (string)$y : $X) === $kh ? ' selected' : '') . '>' . h($X);
        }
        if (is_array($W))
        {
            $H .= '</optgroup>';
    ...
    return$H;
}function html_select($B, $Cf, $Y = "", $wf = TRUE, $pe = "")
{
    if ($wf)
    {
        return"<select name='" . h($B) . "'" . ($pe ? " aria-labelledby='$pe'" : "") . ">" . optionlist($Cf, $Y) . "</select>" . (is_string($wf) ? script("qsl('select').onchange = function () { $wf };", "") : "");
    }
    $H = "";
    foreach ($Cf
as$y => $X)
    {
        $H .= "<label><input type='radio' name='" . h($B) . "' value='" . h($y) . "'" . ($y == $Y ? " checked" : "") . ">" . h($X) . "</label>";
    }

    return$H;
}function select_input($Ka, $Cf, $Y = "", $wf = "", $hg = "")
{
    $bi = ($Cf ? "select" : "input");
...
}function confirm($Qe = "", $lh = "qsl('input')")
{
    return
script("$lh.onclick = function () { return confirm('" . ($Qe ? js_escape($Qe) : lang(0)) . "'); };", "");
}function print_fieldset($t, $xe, $gj = FALSE)
{
...
}function sid()
{
    ...
    return$H;
}function set_password($cj, $M, $V, $E)
{
...
}function get_password()
{
    ...
    if (is_array($H))
    {
        $H = ($_COOKIE["adminer_key"] ? decrypt_string($H[0], $_COOKIE["adminer_key"]) : FALSE);
    }
...
}function get_vals($F, $e = 0)
{
    ...
    return$H;
}function get_key_vals($F, $i = NULL, $th = TRUE)
{
    global$h;
    if (!is_object($i))
    {
    ...
    return$H;
}function get_rows($F, $i = NULL, $n = "<p class='error'>")
{
    global$h;
    $zb = (is_object($i) ? $i : $h);
    $H = [];
    ...
    elseif (!$G && !is_object($i) && $n && defined("PAGE_HEADER"))
    {
        echo$n . error() . "\n";
    }
...
}function where($Z, $p = [])
{
    ...
    foreach ((array)$Z["where"]as$y => $X)
    {
        ...
        {
            $H[] = "$e = " . q($X) . " COLLATE " . charset($h) . "_bin";
        }
    }
...
as$y => $X)
    {
        ...
        if ($Ha)
        {
            $H .= ", $Ha AS " . idf_escape($y);
        }
    }

    return$H;
}function cookie($B, $Y, $_e = 2592000)
{
    global$ba;
...
}function stop_session($fd = FALSE)
{
    $Ui = ini_bool("session.use_cookies");
    if (!$Ui || $fd)
    {
        session_write_close();
        if ($Ui && @ini_set("session.use_cookies", FALSE) === FALSE)
        {
            session_start();
        }
    }
...
}function auth_url($cj, $M, $V, $l = NULL)
{
    global$ic;
...
}function query_redirect($F, $Be, $Qe, $Gg = TRUE, $Gc = TRUE, $Rc = FALSE, $ji = "")
{
    global$h,$n,$b;
    if ($Gc)
    {
...
[implode("\n", $_g),format_time($Ih)];
    ...
    return$h->query($F);
}function apply_queries($F, $S, $Cc = 'table')
{
    foreach ($S
as$Q)
    {
...
as$y => $X)
    ...
    foreach ($Xc["error"]as$y => $n)
    {
        ...
        {
            $Ih = substr($Bb, 0, 3);
            if (function_exists("iconv") && preg_match("~^\xFE\xFF|^\xFF\xFE~", $Ih, $Mg))
            {
                $Bb = iconv("utf-16", "utf-8", $Bb);
            }
            elseif ($Ih == "\xEF\xBB\xBF")
            {
                $Bb = substr($Bb, 3);
            }
            $H .= $Bb . "\n\n";
...
}function shorten_utf8($P, $ye = 80, $Ph = "")
{
...
}function column_foreign_keys($Q)
{
    ...
    return$H;
}function enum_input($T, $Ka, $o, $Y, $xc = NULL)
{
    global$b;
...
}function input($o, $Y, $r)
{
    ...
    {
        echo
h($od[""]) . "<td>" . $b->editInput($_GET["edit"], $o, $Ka, $Y);
    }
    else
    {
...
}function dump_headers($Gd, $Ze = FALSE)
{
    global$b;
    ...
    if ($Pf != "text")
    {
        header("Content-Disposition: attachment; filename=" . $b->dumpFilename($Gd) . ".$H" . ($Pf != "file" && !preg_match('~[^0-9a-z]~', $Pf) ? ".$Pf" : ""));
    }
    session_write_close();
    ob_flush();
    flush();
...
}function file_open_lock($Yc)
{
    ...
    return$md;
}function file_write_unlock($md, $Pb)
{
    rewind($md);
    fwrite($md, $Pb);
...
}function select_value($X, $_, $o, $ii)
{
    global$b;
    if (is_array($X))
    {
...
as$he => $W)
        {
            $H .= "<tr>" . ($X != array_values($X) ? "<th>" . h($he) : "") . "<td>" . select_value($W, $_, $o, $ii);
        }
    ...
    if ($H !== NULL)
    {
        ...
        elseif ($ii != "" && is_shortable($o))
        {
            $H = shorten_utf8($H, max(0, +$ii));
        }
        else
        {
            $H = h($H);
...
as$B => $o)
        {
            ...
            {
                $Wb = $o["default"];
                if ($o["type"] == "bit" && preg_match("~^b'([01]*)'\$~", $Wb, $Mg))
                {
                    $Wb = $Mg[1];
                }
            }
...
} if ($_GET["script"] == "version")
{
    ...
    if ($md)
    {
        file_write_unlock($md, serialize(["signature" => $_POST["signature"],"version" => $_POST["version"]]));
    }
    exit;
}global$b,$h,$m,$ic,$qc,$_c,$n,$od,$ud,$ba,$Vd,$x,$ca,$re,$vf,$gg,$Mh,$zd,$ti,$zi,$U,$Ni,$ia; if (!$_SERVER["REQUEST_URI"])
{
...
}$ba = ($_SERVER["HTTPS"] && strcasecmp($_SERVER["HTTPS"], "off")) || ini_bool("session.cookie_secure");@ini_set("session.use_trans_sid", FALSE); if (!defined("SID"))
{
    ...
    session_start();
}remove_slashes([&$_GET,&$_POST,&$_COOKIE], $Zc); if (get_magic_quotes_runtime())
{
    set_magic_quotes_runtime(FALSE);
}@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode", FALSE);@ini_set("precision", 15);$re = ['en' => 'English','ar' => 'Ø§Ù„Ø¹Ø±Ø¨ÙŠØ©','bg' => 'Ð‘ÑŠÐ»Ð³Ð°Ñ€ÑÐºÐ¸','bn' => 'à¦¬à¦¾à¦‚à¦²à¦¾','bs' => 'Bosanski','ca' => 'CatalÃ ','cs' => 'ÄŒeÅ¡tina','da' => 'Dansk','de' => 'Deutsch','el' => 'Î•Î»Î»Î·Î½Î¹ÎºÎ¬','es' => 'EspaÃ±ol','et' => 'Eesti','fa' => 'ÙØ§Ø±Ø³ÛŒ','fi' => 'Suomi','fr' => 'FranÃ§ais','gl' => 'Galego','he' => '×¢×‘×¨×™×ª','hu' => 'Magyar','id' => 'Bahasa Indonesia','it' => 'Italiano','ja' => 'æ—¥æœ¬èªž','ka' => 'áƒ¥áƒáƒ áƒ—áƒ£áƒšáƒ˜','ko' => 'í•œêµ­ì–´','lt' => 'LietuviÅ³','ms' => 'Bahasa Melayu','nl' => 'Nederlands','no' => 'Norsk','pl' => 'Polski','pt' => 'PortuguÃªs','pt-br' => 'PortuguÃªs (Brazil)','ro' => 'Limba RomÃ¢nÄƒ','ru' => 'Ð ÑƒÑÑÐºÐ¸Ð¹','sk' => 'SlovenÄina','sl' => 'Slovenski','sr' => 'Ð¡Ñ€Ð¿ÑÐºÐ¸','sv' => 'Svenska','ta' => 'à®¤â€Œà®®à®¿à®´à¯','th' => 'à¸ à¸²à¸©à¸²à¹„à¸—à¸¢','tr' => 'TÃ¼rkÃ§e','uk' => 'Ð£ÐºÑ€Ð°Ñ—Ð½ÑÑŒÐºÐ°','vi' => 'Tiáº¿ng Viá»‡t','zh' => 'ç®€ä½“ä¸­æ–‡','zh-tw' => 'ç¹é«”ä¸­æ–‡',];function get_lang()
{
    global$ca;

    return$ca;
}function lang($u, $mf = NULL)
{
    if (is_string($u))
    {
...
}$ca = "en"; if (isset($re[$_COOKIE["adminer_lang"]]))
{
    cookie("adminer_lang", $_COOKIE["adminer_lang"]);
    $ca = $_COOKIE["adminer_lang"];
}
elseif (isset($re[$_SESSION["lang"]]))
{
    $ca = $_SESSION["lang"];
}
else
...
PDO
    {
        ...
        public function __construct()
        {
            ...
            if ($jg !== FALSE)
            {
                unset($b->operators[$jg]);
            }
        }
        public function dsn($nc, $V, $E, $Cf = [])
        {
            try
            {
        ...
        public function query($F, $Hi = FALSE)
        {
            $G = parent::query($F);
            $this->error = "";
            if (!$G)
...
FALSE;
        ...
        public function multi_query($F)
        {
            return$this->_result = $this->query($F);
        }
        public function store_result($G = NULL)
        {
            if (!$G)
            {
                $G = $this->_result;
...
}$ic = [];class Min_SQL
{
    ...
    public function select($Q, $K, $Z, $rd, $Ef = [], $z = 1, $D = 0, $rg = FALSE)
    {
        global$b,$x;
        $ce = (count($rd) < count($K));
        ...
        if (!$F)
        {
            $F = "SELECT" . limit(($_GET["page"] != "last" && $z != "" && $rd && $ce && $x == "sql" ? "SQL_CALC_FOUND_ROWS " : "") . implode(", ", $K) . "\nFROM " . table($Q), ($Z ? "\nWHERE " . implode(" AND ", $Z) : "") . ($rd && $ce ? "\nGROUP BY " . implode(", ", $rd) : "") . ($Ef ? "\nORDER BY " . implode(", ", $Ef) : ""), ($z != "" ? +$z : NULL), ($D ? $z * $D : 0), "\n");
        }
        $Ih = microtime(TRUE);
        $H = $this->_conn->query($F);
        if ($rg)
        {
            echo$b->selectQuery($F, $Ih, !$H);
        }

        return$H;
    }
    public function delete($Q, $Ag, $z = 0)
    {
        $F = "FROM " . table($Q);
...
queries("DELETE" . ($z ? limit1($Q, $F, $Ag) : " $F$Ag"));
    ...
    public function update($Q, $N, $Ag, $z = 0, $L = "\n")
    {
        $aj = [];
        foreach ($N
as$y => $X)
...
queries("INSERT INTO " . table($Q) . ($N ? " (" . implode(", ", array_keys($N)) . ")\nVALUES (" . implode(", ", $N) . ")" : " DEFAULT VALUES"));
    ...
    public function insertUpdate($Q, $J, $pg)
    {
        return
FALSE;
    }
...
queries("ROLLBACK");
    ...
    public function slowQuery($F, $ki)
    {
    }
    ...
    public function quoteBinary($dh)
    {
        return
q($dh);
    }
...
as$A)
        ...
        if (!$H)
        {
            foreach (fields($Q)as$B => $o)
            {
                if ($o["primary"])
                {
                    $H[""] = ["type" => "PRIMARY","columns" => [$B],"lengths" => [],"descs" => [NULL]];
                }
            }
        }
        ...
        foreach (get_rows("PRAGMA index_list(" . table($Q) . ")", $i)as$I)
        {
            ...
            if (!$H[""] || $v["type"] != "UNIQUE" || $v["columns"] != $H[""]["columns"] || $v["descs"] != $H[""]["descs"] || !preg_match("~^sqlite_~", $B))
            {
                $H[$B] = $v;
            }
        }
...
FALSE;
    ...
    function alter_table($Q, $B, $p, $gd, $vb, $yc, $d, $Na, $Zf)
    {
        global$h;
        $Ti = ($Q == "" || $gd);
        foreach ($p
...
FALSE;
            ...
            if ($Q != $B && !queries("ALTER TABLE " . table($Q) . " RENAME TO " . table($B)))
            {
                return
FALSE;
            }
        }
        elseif (!recreate_table($Q, $B, $c, $Nf, $gd, $Na))
        {
            return
FALSE;
        }
        if ($Na)
        {
            ...
            if (!$h->affected_rows)
            {
                queries("INSERT INTO sqlite_sequence (name, seq) VALUES (" . q($B) . ", $Na)");
            }
            queries("COMMIT");
        }
...
TRUE;
    ...
    function recreate_table($Q, $B, $p, $Nf, $gd, $Na, $w = [])
    {
        global$h;
        if ($Q != "")
        {
...
FALSE;
        ...
        if ($Q != "")
        {
            if ($Nf && !queries("INSERT INTO " . table($ei) . " (" . implode(", ", $Nf) . ") SELECT " . implode(", ", array_map('idf_escape', array_keys($Nf))) . " FROM " . table($Q)))
            {
                return
FALSE;
            }
            ...
            if (!queries("DROP TABLE " . table($Q)) || ($Q == $B && !queries("ALTER TABLE " . table($ei) . " RENAME TO " . table($B))) || !alter_indexes($B, $w))
            {
                return
FALSE;
            }
...
[];
    ...
    function set_schema($gh)
    {
        return
TRUE;
    }
    function create_sql($Q, $Na, $Nh)
    {
        global$h;
...
as$y => $X)
                {
                    $Oi[] = "$y = $X";
                    if (isset($pg[idf_unescape($y)]))
                    {
                        $Z[] = "$y = $X";
                    }
                }
                if (!(($Z && queries("UPDATE " . table($Q) . " SET " . implode(", ", $Oi) . " WHERE " . implode(" AND ", $Z)) && $h->affected_rows) || queries("INSERT INTO " . table($Q) . " (" . implode(", ", array_keys($N)) . ") VALUES (" . implode(", ", $N) . ")")))
                {
                    return
FALSE;
                }
...
" . ($B != "" ? "AND relname = " . q($B) : "ORDER BY relname"))as$I)
        {
            $H[$I["Name"]] = $I;
        }
...
ORDER BY a.attnum")as$I)
        {
            ...
            {
                $I["default"] = ($A[1] == "NULL" ? NULL : (($A[1][0] == "'" ? idf_unescape($A[1]) : $A[1]) . $A[2]));
            }
            $H[$I["field"]] = $I;
        }
    ...
    function indexes($Q, $i = NULL)
    {
        ...
        foreach (get_rows("SELECT relname, indisunique::int, indisprimary::int, indkey, indoption , (indpred IS NOT NULL)::int as indispartial FROM pg_index i, pg_class ci WHERE i.indrelid = $Vh AND ci.oid = i.indexrelid", $i)as$I)
        {
            ...
            $H[$Ng]["columns"] = [];
            foreach (explode(" ", $I["indkey"])as$Rd)
            {
                $H[$Ng]["columns"][] = $f[$Rd];
            }
            $H[$Ng]["descs"] = [];
            foreach (explode(" ", $I["indoption"])as$Sd)
            {
                $H[$Ng]["descs"][] = ($Sd & 1 ? '1' : NULL);
            }
            $H[$Ng]["lengths"] = [];
        }
...
[];
    ...
    function error()
    {
        ...
        if (preg_match('~^(.*\n)?([^\n]*)\n( *)\^(\n.*)?$~s', $H, $A))
        {
            $H = $A[1] . preg_replace('~((?:[^&]|&[^;]*;){' . strlen($A[3]) . '})(.*)~', '\1<b>\2</b>', $A[2]) . $A[4];
        }
...
queries("ALTER DATABASE " . idf_escape(DB) . " RENAME TO " . idf_escape($B));
    ...
    function alter_table($Q, $B, $p, $gd, $vb, $yc, $d, $Na, $Zf)
    {
        ...
        if ($Q != "" && $Q != $B)
        {
            $_g[] = "ALTER TABLE " . table($Q) . " RENAME TO " . table($B);
        }
        foreach ($p
as$o)
        {
            ...
            else
            {
                ...
                else
                {
                    if ($e != $X[0])
                    {
                        $_g[] = "ALTER TABLE " . table($B) . " RENAME $e TO $X[0]";
                    }
                    $c[] = "ALTER $e TYPE$X[1]";
                    if (!$X[6])
                    {
                ...
                if ($o[0] != "" || $Zi != "")
                {
                    $_g[] = "COMMENT ON COLUMN " . table($B) . ".$X[0] IS " . ($Zi != "" ? substr($Zi, 9) : "''");
                }
            }
        }
        $c = array_merge($c, $gd);
        if ($Q == "")
        {
            array_unshift($_g, "CREATE TABLE " . table($B) . " (\n" . implode(",\n", $c) . "\n)");
        }
        elseif ($c)
        {
            array_unshift($_g, "ALTER TABLE " . table($Q) . "\n" . implode(",\n", $c));
        }
        if ($Q != "" || $vb != "")
        {
            $_g[] = "COMMENT ON TABLE " . table($B) . " IS " . q($vb);
        }
        if ($Na != "")
        {
        }
...
as$X)
        {
            ...
            {
                $_g[] = "CREATE INDEX " . idf_escape($X[1] != "" ? $X[1] : uniqid($Q . "_")) . " ON " . table($Q) . " (" . implode(", ", $X[2]) . ")";
            }
        }
        if ($Hb)
        {
            array_unshift($_g, "ALTER TABLE " . table($Q) . implode(",", $Hb));
        }
        if ($jc)
        {
            array_unshift($_g, "DROP INDEX " . implode(", ", $jc));
        }
        foreach ($_g
as$F)
        {
...
TRUE;
    ...
    function move_tables($S, $fj, $ci)
    {
        ...
        {
            $O = table_status($Q);
            if (!queries("ALTER " . strtoupper($O["Engine"]) . " " . table($Q) . " SET SCHEMA " . idf_escape($ci)))
            {
                return
FALSE;
            }
...
reset($J);
    ...
    {
        $H = [];
        foreach (get_rows("SELECT * FROM information_schema.triggers WHERE event_object_table = " . q($Q))as$I)
        {
            $H[$I["trigger_name"]] = [$I["action_timing"],$I["event_manipulation"]];
        }
...
0;
    ...
    {
        global$h;
        if (preg_match("~ rows=([0-9]+)~", $h->result("EXPLAIN SELECT * FROM " . idf_escape($R["Name"]) . ($Z ? " WHERE " . implode(" AND ", $Z) : "")), $Mg))
        {
            return$Mg[1];
        }
...
as$Md => $v)
        {
            switch ($v['type']) {case'UNIQUE':$Vg[] = "CONSTRAINT " . idf_escape($Md) . " UNIQUE (" . implode(', ', array_map('idf_escape', $v['columns'])) . ")";

            break;
            case'PRIMARY':$Vg[] = "CONSTRAINT " . idf_escape($Md) . " PRIMARY KEY (" . implode(', ', array_map('idf_escape', $v['columns'])) . ")";
...
as$cd => $bd)
        {
            $Vg[] = "CONSTRAINT " . idf_escape($cd) . " $bd[definition] " . ($bd['deferrable'] ? 'DEFERRABLE' : 'NOT DEFERRABLE');
        }
...
AND c_src.TABLE_NAME = " . q($Q);
        ...
        {
            $H[$I['NAME']] = ["db" => $I['DEST_DB'],"table" => $I['DEST_TABLE'],"source" => [$I['SRC_COLUMN']],"target" => [$I['DEST_COLUMN']],"on_delete" => $I['ON_DELETE'],"on_update" => NULL,];
        }
...
as$y => $X)
                ...
                if (!queries("MERGE " . table($Q) . " USING (VALUES(" . implode(", ", $N) . ")) AS source (c" . implode(", c", range(1, count($N))) . ") ON " . implode(" AND ", $Z) . " WHEN MATCHED THEN UPDATE SET " . implode(", ", $Oi) . " WHEN NOT MATCHED THEN INSERT (" . implode(", ", array_keys($N)) . ") VALUES (" . implode(", ", $N) . ");"))
                {
                    return
FALSE;
                }
...
as$o)
        {
            ...
            else
            {
                ...
                if ($o[0] == "")
                {
                    $c["ADD"][] = "\n  " . implode("", $X) . ($Q == "" ? substr($gd[$X[0]], 16 + strlen($X[0])) : "");
                }
                else
                {
                    unset($X[6]);
...
as$y => $X)
        {
            if (!queries("ALTER TABLE " . idf_escape($B) . " $y" . implode(",", $X)))
            {
                return
FALSE;
            }
...
WHERE sys1.xtype = 'TR' AND sys2.name = " . q($Q))as$I)
        {
...
}$ic['firebird'] = 'Firebird (alpha)'; if (isset($_GET["firebird"]))
{
    $mg = ["interbase"];
...
[];
    ...
    function table_status($B = "", $Sc = FALSE)
    {
        global$h;
        $H = [];
        $Pb = tables_list();
...
ORDER BY r.RDB$FIELD_POSITION';
        ...
        while ($I = ibase_fetch_assoc($G))
        {
            $H[trim($I['FIELD_NAME'])] = ["field" => trim($I["FIELD_NAME"]),"full_type" => trim($I["FIELD_TYPE"]),"type" => trim($I["FIELD_SUB_TYPE"]),"default" => trim($I['FIELD_DEFAULT_VALUE']),"null" => (trim($I["FIELD_NOT_NULL_CONSTRAINT"]) == "YES"),"auto_increment" => '0',"collation" => trim($I["FIELD_COLLATION"]),"privileges" => ["insert" => 1,"select" => 1,"update" => 1],"comment" => trim($I["FIELD_DESCRIPTION"]),];
        }
...
preg_match("~^(columns|sql|status|table)$~", $Tc);
    ...
    $ud = [];
    $qc = [];
}$ic["simpledb"] = "SimpleDB"; if (isset($_GET["simpledb"]))
{
...
as$y => $X)
            ...
            public function _processValue($tc)
            {
...
Min_SQL
    {
        public $pg = "itemName()";
        public function _chunkRequest($Id, $wa, $Tf, $Ic = [])
        {
            global$h;
...
TRUE;
        ...
        public function _extractIds($Q, $Ag, $z)
        {
            $H = [];
            if (preg_match_all("~itemName\\(\\) = (('[^']*+')+)~", $Ag, $Ie))
            {
                $H = array_map('idf_unescape', $Ie[1]);
            }
            else
            {
                foreach (sdb_request_all('Select', 'Item', ['SelectExpression' => 'SELECT itemName() FROM ' . table($Q) . $Ag . ($z ? " LIMIT 1" : "")])as$fe)
                {
                    $H[] = $fe->Name;
                }
            }
...
as$N)
            {
                if (!$this->update($Q, $N, "WHERE `itemName()` = " . q($N["`itemName()`"])))
                {
                    return
FALSE;
                }
...
[$l => count(tables_list())];
    ...
    function hmac($Ca, $Pb, $y, $Eg = FALSE)
    {
        $Wa = 64;
        if (strlen($y) > $Wa)
        {
...
FALSE;
    ...
    function sdb_request_all($wa, $bi, $Tf = [], $ki = 0)
    {
        $H = [];
...
FALSE;
            ...
            if ($z)
            {
                $Tf['SelectExpression'] = preg_replace('~\d+\s*$~', $z - count($H), $Tf['SelectExpression']);
            }
        } while ($sj->NextToken);
    ...
    $ud = ["count"];
    $qc = [["json"]];
}$ic["mongo"] = "MongoDB"; if (isset($_GET["mongo"]))
{
    $mg = ["mongo","mongodb"];
    define("DRIVER", "mongo");
...
as$l)
            {
                $H[$l] = count($h->_link->selectDB($l)->getCollectionNames(TRUE));
            }
...
as$y => $X)
        ...
        function sql_query_where_parser($Ag)
        {
...
as$X)
        {
            ...
            if ($N == "DROP")
            {
                $H = $h->_db->command(["deleteIndexes" => $Q,"index" => $B]);
            }
            else
            {
                $f = [];
...
}$ic["elastic"] = "Elasticsearch (beta)"; if (isset($_GET["elastic"]))
{
    ...
    if (function_exists('json_decode') && ini_bool('allow_url_fopen'))
    {
        class Min_DB
        {
            ...
            public $errorvar;
            public $_urlvar;
            public function rootQuery($dg, $Bb = [], $We = 'GET')
            {
                @ini_set('track_errors', 1);
...
FALSE;
            ...
            public function query($dg, $Bb = [], $We = 'GET')
            {
...
as$X)
            {
                ...
                elseif ($ob . $X != "")
                {
                    ...
                    if ($yf == "=")
                    {
                        $Pb["query"]["filtered"]["filter"]["and"][] = $fi;
                    }
                    else
                    {
                        $Pb["query"]["filtered"]["query"]["bool"]["must"][] = $fi;
                    }
                }
            }
            ...
            if ($rg)
            {
                echo$b->selectQuery("$F: " . json_encode($Pb), $Ih, !$hh);
            }
            if (!$hh)
            {
                return
...
Min_Result($H);
        ...
        public function update($T, $Fg, $Ag, $z = 0, $L = "\n")
        {
            $bg = preg_split('~ *= *~', $Ag);
            if (count($bg) == 2)
            {
...
FALSE;
        ...
        public function delete($T, $Ag, $z = 0)
        {
            $Id = [];
...
as$Pd => $Jh)
    ...
    function tables_list()
    {
        ...
        if ($H)
        {
            $H = array_fill_keys(array_keys($H[$h->_db]["mappings"]), 'table');
        }
...
[["type" => "PRIMARY","columns" => ["_id"]],];
    ...
    function fields($Q)
    {
        ...
        if ($G)
        {
            ...
            if (!$Ee)
            {
                $Ee = $G[$h->_db]['mappings'][$Q]['properties'];
            }
            if ($Ee)
            {
                foreach ($Ee
...
as$Q)
        {
            $H = $H && $h->query(urlencode($Q), [], 'DELETE');
        }
...
}$ic["clickhouse"] = "ClickHouse (alpha)"; if (isset($_GET["clickhouse"]))
{
...
FALSE;
            ...
            if ($H === NULL)
            {
                if (!$this->isQuerySelectLike($F) && $Xc === '')
                {
                    return
TRUE;
                }
...
Min_Result($H);
        ...
        public function isQuerySelectLike($F)
        {
...
FALSE;
    ...
    function unconvert_field($o, $H)
    {
        if (in_array($o['type'], ["Int8","Int16","Int32","Int64","UInt8","UInt16","UInt32","UInt64","Float32","Float64"]))
        {
            return"to$o[type]($H)";
        }
...
preg_match("~^(columns|sql|status|table|drop_col)$~", $Tc);
    ...
    $ud = ["avg","count","count distinct","max","min","sum"];
    $qc = [];
}$ic = ["server" => "MySQL"] + $ic; if (!defined("DRIVER"))
{
...
MySQLi
        {
            ...
            public function connect($M = "", $V = "", $E = "", $j = NULL, $ig = NULL, $zh = NULL)
            {
                global$b;
                ...
                if ($Hh)
                {
                    $this->ssl_set($Hh['key'], $Hh['cert'], $Hh['ca'], '', '');
                }
            ...
            public function set_charset($db)
            {
...
Min_PDO
        {
            ...
            public function connect($M, $V, $E)
            {
                ...
                if ($Hh)
                {
                    if (!empty($Hh['key']))
                    {
                        $Cf[PDO::MYSQL_ATTR_SSL_KEY] = $Hh['key'];
                    }
                    if (!empty($Hh['cert']))
                    {
                        $Cf[PDO::MYSQL_ATTR_SSL_CERT] = $Hh['cert'];
                    }
                    if (!empty($Hh['ca']))
                    {
                        $Cf[PDO::MYSQL_ATTR_SSL_CA] = $Hh['ca'];
                    }
                }
...
queries($ng . implode(",\n", $aj) . $Ph);
        ...
        public function slowQuery($F, $ki)
        {
            if (min_version('5.7.8', '10.1.2'))
            {
                ...
                {
                    return"SET STATEMENT max_statement_time=$ki FOR $F";
                }
                elseif (preg_match('~^(SELECT\b)(.+)~is', $F, $A))
                {
                    return"$A[1] /*+ MAX_EXECUTION_TIME(" . ($ki * 1000) . ") */ $A[2]";
                }
            }
        }
        public function convertSearch($u, $X, $o)
...
strtolower(($Fe ? "information-schema-$B-table/" : str_replace("_", "-", $B) . "-table.html"));
            ...
            if (DB == "mysql")
            {
                return($Fe ? "mysql$B-table/" : "system-database.html");
            }
        }
    }
    function idf_escape($u)
...
Min_DB;
        ...
        $H = $h->error;
        if (function_exists('iconv') && !is_utf8($H) && strlen($dh = iconv("windows-1250", "utf-8", $H)) > strlen($H))
        {
            $H = $dh;
        }
...
limit($F, $Z, 1, 0, $L);
    ...
    function db_collation($l, $qb)
    {
        ...
        if (preg_match('~ COLLATE ([^ ]+)~', $Hb, $A))
        {
            $H = $A[1];
        }
        elseif (preg_match('~ CHARACTER SET ([^ ]+)~', $Hb, $A))
        {
            $H = $qb[$A[1]][-1];
        }
...
as$l)
    ...
    function table_status($B = "", $Sc = FALSE)
    {
        ...
        foreach (get_rows($Sc && min_version(5) ? "SELECT TABLE_NAME AS Name, ENGINE AS Engine, TABLE_COMMENT AS Comment FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() " . ($B != "" ? "AND TABLE_NAME = " . q($B) : "ORDER BY Name") : "SHOW TABLE STATUS" . ($B != "" ? " LIKE " . q(addcslashes($B, "%_\\")) : ""))as$I)
        {
            if ($I["Engine"] == "InnoDB")
            {
                $I["Comment"] = preg_replace('~(?:(.+); )?InnoDB free: .*~', '\1', $I["Comment"]);
            }
            if (!isset($I["Engine"]))
            {
                $I["Comment"] = "";
...
queries("CREATE DATABASE " . idf_escape($l) . ($d ? " COLLATE " . q($d) : ""));
    ...
    function rename_database($B, $d)
    {
        ...
        if (create_database($B, $d))
        {
            ...
            foreach (tables_list()as$Q => $T)
            {
                $Pg[] = table($Q) . " TO " . idf_escape($B) . "." . table($Q);
            }
...
queries("DROP TABLE " . implode(", ", array_map('table', $S)));
    ...
    function move_tables($S, $fj, $ci)
    {
        ...
        foreach (array_merge($S, $fj)as$Q)
        {
            $Pg[] = table($Q) . " TO " . idf_escape($ci) . "." . table($Q);
        }

        return
queries("RENAME TABLE " . implode(", ", $Pg));
    }
    function copy_tables($S, $fj, $ci)
    {
...
as$Q)
        {
            ...
            if (($_POST["overwrite"] && !queries("\nDROP TABLE IF EXISTS $B")) || !queries("CREATE TABLE $B LIKE " . table($Q)) || !queries("INSERT INTO $B SELECT * FROM " . table($Q)))
            {
                return
FALSE;
            }
...
as$Q)
        {
            $B = ($ci == DB ? table("copy_$Q") : idf_escape($ci) . "." . table($Q));
            $ej = view($Q);
            if (($_POST["overwrite"] && !queries("DROP VIEW IF EXISTS $B")) || !queries("CREATE VIEW $B AS $ej[select]"))
            {
                return
FALSE;
            }
...
as$Sf)
        {
            $p[] = ["field" => str_replace("``", "`", $Sf[2]) . $Sf[3],"type" => strtolower($Sf[5]),"length" => preg_replace_callback("~$_c~s", 'normalize_enum', $Sf[6]),"unsigned" => strtolower(preg_replace('~\s+~', ' ', trim("$Sf[8] $Sf[7]"))),"null" => 1,"full_type" => $Sf[4],"inout" => strtoupper($Sf[1]),"collation" => strtolower($Sf[9]),];
        }
        if ($T != "FUNCTION")
        {
            return
...
TRUE;
    ...
    {
        $H = "";
        foreach (get_rows("SHOW TRIGGERS LIKE " . q(addcslashes($Q, "%_\\")), NULL, "-- ")as$I)
        {
...
get_key_vals("SHOW STATUS");
    ...
    function unconvert_field($o, $H)
    {
        ...
        if (preg_match("~geometry|point|linestring|polygon~", $o["type"]))
        {
            $H = (min_version(8) ? "ST_" : "") . "GeomFromText($H, SRID($o[field]))";
        }
...
}define("SERVER", $_GET[DRIVER]);define("DB", $_GET["db"]);define("ME", str_replace(":", "%3a", preg_replace('~\?.*~', '', relative_uri())) . '?' . (sid() ? SID . '&' : '') . (SERVER !== NULL ? DRIVER . "=" . urlencode(SERVER) . '&' : '') . (isset($_GET["username"]) ? "username=" . urlencode($_GET["username"]) . '&' : '') . (DB != "" ? 'db=' . urlencode(DB) . '&' . (isset($_GET["ns"]) ? "ns=" . urlencode($_GET["ns"]) . "&" : "") : ''));$ia = "4.7.7";class Adminer
{
    public $operators;
...
[SERVER,$_GET["username"],get_password()];
    ...
    public function permanentLogin($Hb = FALSE)
    {
        return
password_file($Hb);
    }
...
DB;
    ...
    public function databases($ed = TRUE)
    {
        return
get_databases($ed);
    }
...
TRUE;
    ...
    public function css()
    {
        ...
        if (file_exists($Yc))
        {
            $H[] = "$Yc?v=" . crc32(file_get_contents($Yc));
        }
    ...
    public function loginFormField($B, $Ad, $Y)
    {
        return$Ad . $Y;
    }
    public function login($Ce, $E)
    {
        if ($E == "")
        {
            return
...
TRUE;
    ...
    public function tableName($Th)
    {
        return
h($Th["Name"]);
    }
    public function fieldName($o, $Ef = 0)
    {
        return'<span title="' . h($o["full_type"]) . '">' . h($o["field"]) . '</span>';
    }
    public function selectLinks($Th, $N = "")
    {
        global$x,$m;
        echo'<p class="links">';
        $Ae = ["select" => lang(42)];
...
as$y => $X)
        {
            echo" <a href='" . h(ME) . "$y=" . urlencode($B) . ($y == "edit" ? $N : "") . "'" . bold(isset($_GET[$y])) . ">$X</a>";
        }
        echo
...
foreign_keys($Q);
    ...
    public function backwardKeys($Q, $Sh)
    {
        return
[];
    }
    public function backwardKeysPrint($Qa, $I)
    {
    }
    public function selectQuery($F, $Ih, $Rc = FALSE)
    {
        global$x,$m;
        $H = "</p>\n";
        if (!$Rc && ($ij = $m->warnings()))
    ...
    public function sqlCommandQuery($F)
    {
        return
shorten_utf8(trim($F), 1000);
    ...
    public function rowDescriptions($J, $hd)
    {
        return$J;
    }
    ...
    public function selectVal($X, $_, $o, $Mf)
    {
...
as$B => $v)
        {
            ...
            foreach ($v["columns"]as$y => $X)
            {
                $rg[] = "<i>" . h($X) . "</i>" . ($v["lengths"][$y] ? "(" . $v["lengths"][$y] . ")" : "") . ($v["descs"][$y] ? " DESC" : "");
            }
            echo"<tr title='" . h($B) . "'><th>$v[type]<td>" . implode(", ", $rg) . "\n";
        }
        echo"</table>\n";
    }
    public function selectColumnsPrint($K, $f)
    {
        global$od,$ud;
...
as$s => $v)
    ...
    public function selectOrderPrint($Ef, $f, $w)
    {
    ...
    public function selectLengthPrint($ii)
    {
        if ($ii !== NULL)
        {
...
as$s => $v)
        {
            ...
            {
                $H[] = "MATCH (" . implode(", ", array_map('idf_escape', $v["columns"])) . ") AGAINST (" . q($_GET["fulltext"][$s]) . (isset($_GET["boolean"][$s]) ? " IN BOOLEAN MODE" : "") . ")";
            }
        }
        foreach ((array)$_GET["where"]as$y => $X)
        {
            if ("$X[col]$X[val]" != "" && in_array($X["op"], $this->operators))
            {
                ...
                elseif ($X["op"] == "LIKE %%")
                {
                    $yb = " LIKE " . $this->processInput($p[$X["col"]], "%$X[val]%");
                }
                elseif ($X["op"] == "ILIKE %%")
                {
                    $yb = " ILIKE " . $this->processInput($p[$X["col"]], "%$X[val]%");
                }
                ...
                {
                    $yb .= " " . $this->processInput($p[$X["col"]], $X["val"]);
                }
                if ($X["col"] != "")
                {
                    $H[] = $ng . $m->convertSearch(idf_escape($X["col"]), $X, $p[$X["col"]]) . $yb;
                }
                else
                {
                    $sb = [];
...
as$B => $o)
                    {
                        ...
                        {
                            $sb[] = $ng . $m->convertSearch(idf_escape($B), $X, $o) . $yb;
                        }
                    }
    ...
    public function selectOrderProcess($p, $w)
    {
        ...
        foreach ((array)$_GET["order"]as$y => $X)
        {
            if ($X != "")
            {
                $H[] = (preg_match('~^((COUNT\(DISTINCT |[A-Z0-9_]+\()(`(?:[^`]|``)+`|"(?:[^"]|"")+")\)|COUNT\(\*\))$~', $X) ? $X : idf_escape($X)) . (isset($_GET["desc"][$y]) ? " DESC" : "");
            }
        }
    ...
    public function selectEmailProcess($Z, $hd)
    {
        return
FALSE;
    }
    public function selectQueryBuild($K, $Z, $rd, $Ef, $z, $D)
    {
        return"";
    }
    public function messageQuery($F, $ji, $Rc = FALSE)
    {
        global$x,$m;
        restart_session();
        ...
        if (strlen($F) > 1e6)
        {
            $F = preg_replace('~[\x80-\xFF]+$~', '', substr($F, 0, 1e6)) . "\nâ€¦";
        }
...
explode("/", $H);
    ...
    public function editInput($Q, $o, $Ka, $Y)
    {
        if ($o["type"] == "enum")
        {
    ...
    public function processInput($o, $Y, $r = "")
    {
        ...
        {
            $H = idf_escape($B) . " $r " . (preg_match("~^(\\d+|'[0-9.: -]') [A-Z_]+\$~i", $Y) ? $Y : $H);
        }
...
['sql' => 'SQL','csv' => 'CSV,','csv;' => 'CSV;','tsv' => 'TSV'];
    ...
    public function dumpTable($Q, $Nh, $ee = 0)
    {
        if ($_POST["format"] != "sql")
        {
            echo"\xef\xbb\xbf";
        ...
        else
        {
            ...
            {
                $Hb = create_sql($Q, $_POST["auto_increment"], $Nh);
            }
            set_utf8mb4($Hb);
            if ($Nh && $Hb)
            {
                if ($Nh == "DROP+CREATE" || $ee == 1)
                {
                    echo"DROP " . ($ee == 2 ? "VIEW" : "TABLE") . " IF EXISTS " . table($Q) . ";\n";
                }
                if ($ee == 1)
                {
                    $Hb = remove_definer($Hb);
                }
                echo"$Hb;\n\n";
            }
        }
    }
    public function dumpData($Q, $Nh, $F)
    {
        global$h,$x;
...
as$X)
                    ...
                    else
                    {
                        if (!$Xd)
                        {
                            $Xd = "INSERT INTO " . table($Q) . " (" . implode(", ", array_map('idf_escape', $le)) . ") VALUES";
                        }
                        foreach ($I
as$y => $X)
                        {
                        ...
                        elseif (strlen($Za) + 4 + strlen($dh) + strlen($Ph) < $Ke)
                        {
                            $Za .= ",$dh";
                        }
                        else
    ...
    public function dumpFilename($Gd)
    {
        return
friendly_url($Gd != "" ? $Gd : (SERVER != "" ? SERVER : "localhost"));
    }
    public function dumpHeaders($Gd, $Ze = FALSE)
    {
        $Pf = $_POST["output"];
...
as$V => $E)
                    {
                        if ($E !== NULL)
                        {
                            ...
                            {
                                $Pf .= "<li><a href='" . h(auth_url($cj, $M, $V, $l)) . "'>($ic[$cj]) " . h($V . ($M != "" ? "@" . $this->serverName($M) : "") . ($l != "" ? " - $l" : "")) . "</a>\n";
                            }
                        }
                    }
                }
...
';
    ...
    public function databasesPrint($Ye)
    {
        global$b,$h;
        $k = $this->databases();
        if ($k && !in_array(DB, $k))
...
{
    $b->operators = $_f;
}function page_header($mi, $n = "", $Ya = [], $ni = "")
{
    global$ca,$ia,$b,$ic,$x;
    page_headers();
    if (is_ajax() && $n)
...
-----END PUBLIC KEY-----
";
        if (openssl_verify($dj["version"], base64_decode($dj["signature"]), $yg) == 1)
        {
            $_COOKIE["adminer_version"] = $dj["version"];
        }
    }
    echo'<script',nonce(),'>
...
as$y => $X)
                {
                    ...
                    if ($bc != "")
                    {
                        echo"<a href='" . h(ME . "$y=") . urlencode(is_array($X) ? $X[0] : $X) . "'>$bc</a> &raquo; ";
                    }
                }
            }
            echo"$mi\n";
...
FALSE;
    ...
    return
long2str($W, TRUE);
}$h = '';$zd = $_SESSION["token"]; if (!$zd)
{
    $_SESSION["token"] = rand(1, 1e6);
}$ti = get_token();$gg = []; if ($_COOKIE["adminer_permanent"])
{
...
}$La = $_POST["auth"]; if ($La)
{
    ...
    if (count($_POST) == 1 || DRIVER != $cj || SERVER != $M || $_GET["username"] !== $V || DB != $l)
    {
        redirect(auth_url($cj, $M, $V, $l));
    }
}
elseif ($_POST["logout"])
{
...
as$y => $X)
    {
        list($cj, $M, $V, $l) = array_map('base64_decode', explode("-", $y));
        if ($cj == DRIVER && $M == SERVER && $V == $_GET["username"] && $l == DB)
        {
            unset($gg[$y]);
        }
    }
...
}function auth_error($n)
{
    ...
    if (!$_COOKIE[$sh] && $_GET[$sh] && ini_bool("session.use_only_cookies"))
    {
        $n = lang(89);
    }
...
}stop_session(TRUE); if (isset($_GET["username"]) && is_string(get_password()))
{
    ...
    if (is_numeric($ig) && ($ig < 1024 || $ig > 65535))
    {
        auth_error(lang(93));
    }
    check_invalid_login();
    ...
    $m = new
Min_Driver($h);
}$Ce = NULL; if (!is_object($h) || ($Ce = $b->login($_GET["username"], get_password())) !== TRUE)
{
...
}function select($G, $i = NULL, $Hf = [], $z = 0)
{
    global$x;
    $Ae = [];
    $w = [];
    ...
    for ($s = 0;(!$z || $s < $z) && ($I = $G->fetch_row());$s++)
    {
        if (!$s)
        {
            ...
            for ($ge = 0;$ge < count($I);$ge++)
            {
                ...
                if ($Hf && $x == "sql")
                {
                    $Ae[$ge] = ($B == "table" ? "table=" : ($B == "possible_keys" ? "indexes=" : NULL));
                }
                elseif ($Gf != "")
                {
                    if (!isset($w[$Gf]))
...
as$y => $X)
        {
            ...
            if (isset($Ae[$y]) && !$f[$Ae[$y]])
            {
                ...
                else
                {
                    ...
                    foreach ($w[$Ae[$y]]as$ob => $ge)
                    {
                        $_ .= "&where" . urlencode("[" . bracket_escape($ob) . "]") . "=" . urlencode($I[$ge]);
                    }
                }
                $X = "<a href='" . h(ME . $_) . "'>$X</a>";
            }
    ...
    return$H;
}function referencable_primary($mh)
{
    $H = [];
...
}function adminer_setting($y)
{
    ...
    return$uh[$y];
}function set_adminer_settings($uh)
{
    return
cookie("adminer_settings", http_build_query($uh + adminer_settings()));
}function textarea($B, $Y, $J = 10, $sb = 80)
{
    global$x;
...
h($Y);
    ...
    echo"</textarea>";
}function edit_type($y, $o, $qb, $id = [], $Pc = [])
{
    global$Mh,$U,$Ni,$vf;
    $T = $o["type"];
    echo'<td><select name="',h($y),'[type]" class="type" aria-labelledby="label-type">';
    if ($T && !isset($U[$T]) && !isset($id[$T]) && !in_array($T, $Pc))
    {
        $Pc[] = $T;
    }
    if ($id)
...
}function edit_fields($p, $qb, $T = "TABLE", $id = [])
{
    global$Vd;
    $p = array_values($p);
...
',($T == "PROCEDURE" ? "<td>" . html_select("fields[$s][inout]", explode("|", $Vd), $o["inout"]) : ""),'<th>';
        ...
        echo"<td>",(support("move_col") ? "<input type='image' class='icon' name='add[$s]' src='" . h(preg_replace("~\\?.*~", "", ME) . "?file=plus.gif&version=4.7.7") . "' alt='+' title='" . lang(108) . "'> " . "<input type='image' class='icon' name='up[$s]' src='" . h(preg_replace("~\\?.*~", "", ME) . "?file=up.gif&version=4.7.7") . "' alt='â†‘' title='" . lang(109) . "'> " . "<input type='image' class='icon' name='down[$s]' src='" . h(preg_replace("~\\?.*~", "", ME) . "?file=down.gif&version=4.7.7") . "' alt='â†“' title='" . lang(110) . "'> " : ""),($If == "" || support("drop_col") ? "<input type='image' class='icon' name='drop_col[$s]' src='" . h(preg_replace("~\\?.*~", "", ME) . "?file=cross.gif&version=4.7.7") . "' alt='x' title='" . lang(111) . "'>" : "");
    }
...
TRUE;
    ...
    if ($ug == ["ALL PRIVILEGES","GRANT OPTION"])
    {
        return($pd == "GRANT" ? queries("$pd ALL PRIVILEGES$uf WITH GRANT OPTION") : queries("$pd ALL PRIVILEGES$uf") && queries("$pd GRANT OPTION$uf"));
    }

    return
queries("$pd " . preg_replace('~(GRANT OPTION)\([^)]*\)~', '\1', implode("$f, ", $ug) . $f) . $uf);
}function drop_create($jc, $Hb, $kc, $gi, $mc, $Be, $Te, $Re, $Se, $rf, $ef)
{
    if ($_POST["drop"])
    {
        query_redirect($jc, $Be, $Te);
    }
    elseif ($rf == "")
    {
        query_redirect($Hb, $Be, $Se);
    }
    elseif ($rf != $ef)
    {
        $Jb = queries($Hb);
...
case'k':$X *= 1024;}
    ...
    return$X;
}function doc_link($eg, $hi = "<sup>?</sup>")
{
    global$x,$h;
    $qh = $h->server_info;
...
}$vf = "RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";class TmpFile
{
    public $handler;
    public $size;
    public function __construct()
...
}$_c = "'(?:''|[^'\\\\]|\\\\.)*'";$Vd = "IN|OUT|INOUT"; if (isset($_GET["select"]) && ($_POST["edit"] || $_POST["clone"]) && !$_POST["save"])
{
...
as$y => $X)
            {
                echo"<tr valign='top'><td>" . h($X[0]) . "<td>" . h($X[1]) . "<th>" . h($y) . "<td><a href='" . h(ME . 'trigger=' . urlencode($a) . '&name=' . urlencode($y)) . "'>" . lang(136) . "</a>\n";
            }
            echo"</table>\n";
        }
...
as$s => $A)
    ...
    foreach (table_status('', TRUE)as$Q => $R)
    {
        ...
        foreach ($b->foreignKeys($Q)as$X)
        {
            ...
            {
                $ue = $Sa;
                if ($Wh[$Q][1] || $Wh[$X["table"]][1])
                {
                    $ue = min(floatval($Wh[$Q][1]), floatval($Wh[$X["table"]][1])) - 1;
                }
                else
                {
                    $Sa -= .1;
...
TmpFile;
                            ...
                            if ($de && $_POST["triggers"] && $Q && ($Ei = trigger_sql($B)))
                            {
                                echo"\nDELIMITER ;;\n$Ei\nDELIMITER ;\n";
                            }
                            if ($Mc == "tar")
...
as$ej)
                    {
                        $b->dumpTable($ej, $_POST["table_style"], 1);
                    }
                    if ($Mc == "tar")
                    {
                        echo
...
';
    ...
    if (!$I)
    {
        $I = ["output" => "text","format" => "sql","db_style" => (DB != "" ? "" : "CREATE"),"table_style" => "DROP+CREATE","data_style" => "INSERT"];
    }
    if (!isset($I["events"]))
    {
...
elseif (isset($_GET["privileges"]))
{
    ...
    if (!$G)
    {
        $G = $h->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");
    }
    echo"<form action=''><p>\n";
    hidden_fields_get();
    ...
    while ($I = $G->fetch_assoc())
    {
        echo'<tr' . odd() . '><td>' . h($I["User"]) . "<td>" . h($I["Host"]) . '<td><a href="' . h(ME . 'user=' . urlencode($I["User"]) . '&host=' . urlencode($I["Host"])) . '">' . lang(10) . "</a>\n";
    }
    if (!$pd || DB != "")
    {
...
elseif (isset($_GET["sql"]))
{
    ...
    if (!$n && $_POST)
    {
        ...
        if (is_string($F))
        {
            ...
            {
                @ini_set("memory_limit", max(ini_bytes("memory_limit"), 2 * strlen($F) + memory_get_usage() + 8e6));
            }
            if ($F != "" && strlen($F) < 1e6)
            {
                $zg = $F . (preg_match("~;[ \t\r\n]*\$~", $F) ? "" : ";");
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                $Ih = microtime(TRUE);
                                if ($h->multi_query($zg) && is_object($i) && preg_match("~^$Bh*+USE\\b~i", $zg))
                                {
                                    $i->query($zg);
                                }
                                do
...
2;
                                    ...
                                    else
                                    {
                                        ...
                                        if ($ij)
                                        {
                                            $ji .= ", <a href='#$jj'>" . lang(47) . "</a>" . script("qsl('a').onclick = partial(toggle, '$jj');", "");
                                        }
                                        $Jc = NULL;
                                        $Kc = "explain-$ub";
                                        if (is_object($G))
                                        {
                                            ...
                                            if (!$_POST["only_errors"])
                                            {
                                                ...
                                                if ($i && preg_match("~^($Bh|\\()*+SELECT\\b~i", $zg) && ($Jc = explain($i, $zg)))
                                                {
                                        ...
                                        else
                                        {
                                            ...
                                            if (!$_POST["only_errors"])
                                            {
                                                echo"<p class='message' title='" . h($h->info) . "'>" . lang(151, $_a) . "$ji\n";
                                            }
                                        }
            ...
            elseif ($Bc && $ub > 1)
            {
                echo"<p class='error'>" . lang(148) . ": " . implode("", $Bc) . "\n";
            }
        }
        else
        {
...
as$B => $o)
    {
        if (!isset($o["privileges"][$Oi ? "update" : "insert"]) || $b->fieldName($o) == "" || $o["generated"])
        {
            unset($p[$B]);
        }
    }
    if ($_POST && !$n && !isset($_GET["select"]))
    {
        ...
        if ($_POST["insert"])
        {
            $Be = ($Oi ? NULL : $_SERVER["REQUEST_URI"]);
        }
        elseif (!preg_match('~^.+&select=.+$~', $Be))
        {
            $Be = ME . "select=" . urlencode($a);
        }
        $w = indexes($a);
        ...
        {
            queries_redirect($Be, lang(166), $m->delete($a, $Bg, !$Ji));
        }
        else
        {
            $N = [];
...
as$Uh => $o)
    {
        $id[str_replace("`", "``", $Uh) . "`" . str_replace("`", "``", $o["field"])] = $Uh;
    }
    $Kf = [];
    $R = [];
    if ($a != "")
        ...
        else
        {
            ...
            foreach ($I["fields"]as$y => $o)
            {
                ...
                if ($o["field"] != "")
                {
                    ...
                    if ($q !== NULL)
                    {
                        $gd[idf_escape($o["field"])] = ($a != "" && $x != "sqlite" ? "ADD" : " ") . format_foreign_key(['table' => $id[$o["type"]],'source' => [$o["field"]],'target' => [$Fi["field"]],'on_delete' => $o["on_delete"],]);
                    }
...
as$l)
    ...
    elseif (DB != "")
    {
        $I["collation"] = db_collation(DB, $qb);
    }
    elseif ($x == "sql")
    {
...
';
    ...
    {
        echo"<input type='image' class='icon' name='add' src='" . h(preg_replace("~\\?.*~", "", ME) . "?file=plus.gif&version=4.7.7") . "' alt='+' title='" . lang(108) . "'>\n";
    }
    echo'<input type="hidden" name="token" value="',$ti,'">
</form>
';
...
elseif (isset($_GET["scheme"]))
{
    ...
    if ($_POST && !$n)
    {
        ...
        if ($_POST["drop"])
        {
            query_redirect("DROP SCHEMA " . idf_escape($_GET["ns"]), $_, lang(188));
        }
        else
        {
            $B = trim($I["name"]);
            ...
            if ($_GET["ns"] == "")
            {
                query_redirect("CREATE SCHEMA " . idf_escape($B), $_, lang(189));
            }
            elseif ($_GET["ns"] != $B)
            {
                query_redirect("ALTER SCHEMA " . idf_escape($_GET["ns"]) . " RENAME TO " . idf_escape($B), $_, lang(190));
            }
            else
            {
                redirect($_);
...
elseif (isset($_GET["call"]))
{
    ...
    foreach ($Zg["fields"]as$s => $o)
    {
        ...
        {
            $Of[$s] = "@" . idf_escape($o["field"]) . " AS " . idf_escape($o["field"]);
        }
    ...
    if (!$n && $_POST)
    {
        ...
        else
        {
            ...
            if ($Of)
            {
                select($h->query("SELECT " . implode(", ", $Of)));
            }
        }
    }
    echo'
...
elseif (isset($_GET["foreign"]))
{
    ...
    if ($_POST && !$n && !$_POST["add"] && !$_POST["change"] && !$_POST["change-js"])
    {
        ...
        if ($x == "sqlite")
        {
            queries_redirect($Be, $Qe, recreate_table($a, $a, [], [], [" $B" => ($_POST["drop"] ? "" : " " . format_foreign_key($I))]));
        }
        else
        {
...
';
    ...
    {
        echo
lang(78) . ": " . html_select("ns", $b->schemas(), $I["ns"] != "" ? $I["ns"] : $_GET["ns"], $wf);
    }
    elseif ($x != "sqlite")
    {
        $Ub = [];
...
elseif (isset($_GET["view"]))
{
    ...
    if ($_POST && !$n)
    {
        ...
        if (!$_POST["drop"] && $a == $B && $x != "sqlite" && $T == "VIEW" && $Lf == "VIEW")
        {
            query_redirect(($x == "mssql" ? "ALTER" : "CREATE OR REPLACE") . " VIEW " . table($B) . $Ha, $Be, $Qe);
        }
        else
        {
...
elseif (isset($_GET["event"]))
{
    ...
    if ($_POST && !$n)
    {
        ...
        elseif (in_array($I["INTERVAL_FIELD"], $Yd) && isset($Kh[$I["STATUS"]]))
        {
...
elseif (isset($_GET["sequence"]))
{
    ...
    if ($_POST && !$n)
    {
        ...
        if ($_POST["drop"])
        {
            query_redirect("DROP SEQUENCE " . idf_escape($fa), $_, lang(222));
        }
        elseif ($fa == "")
        {
            query_redirect("CREATE SEQUENCE " . idf_escape($B), $_, lang(223));
        }
        elseif ($fa != $B)
        {
            query_redirect("ALTER SEQUENCE " . idf_escape($fa) . " RENAME TO " . idf_escape($B), $_, lang(224));
        }
        else
        {
            redirect($_);
...
elseif (isset($_GET["type"]))
{
    ...
    if ($_POST && !$n)
    {
        ...
        if ($_POST["drop"])
        {
            query_redirect("DROP TYPE " . idf_escape($ga), $_, lang(227));
        }
        else
        {
    ...
    if ($_POST)
    {
        if (!$n && in_array($_POST["Timing"], $Di["Timing"]) && in_array($_POST["Event"], $Di["Event"]) && in_array($_POST["Type"], $Di["Type"]))
        {
            ...
            else
            {
                ...
                if ($B != "")
                {
                    queries(create_trigger($uf, $I + ["Type" => reset($Di["Type"])]));
                }
            }
        }
        $I = $_POST;
...
elseif (isset($_GET["user"]))
{
    ...
    {
        foreach (explode(",", ($I["Privilege"] == "Grant option" ? "" : $I["Context"]))as$Db)
        {
            $ug[$Db][$I["Privilege"]] = $I["Comment"];
        }
    }
    ...
    {
        $ug["Columns"][$X] = $ug["Tables"][$X];
    }
    unset($ug["Server Admin"]["Usage"]);
    foreach ($ug["Tables"]as$y => $X)
    {
        unset($ug["Databases"][$y]);
    }
    $df = [];
    if ($_POST)
    {
        ...
        {
            $df[$X] = (array)$df[$X] + (array)$_POST["grants"][$y];
        }
    }
    $qd = [];
    $sf = "";
...
as$X)
    ...
    if ($_POST && !$n)
    {
        ...
        if ($_POST["drop"])
        {
            query_redirect("DROP USER $tf", ME . "privileges=", lang(238));
        }
        else
        {
            ...
            if (!$n)
            {
                ...
                elseif ($cg != $sf)
                {
                    queries("SET PASSWORD FOR $ff = " . q($cg));
                }
            }
            if (!$n)
            {
...
as$nf => $pd)
                {
                    ...
                    if (isset($_GET["grant"]))
                    {
                        $Wg = array_diff(array_keys(array_filter($df[$nf], 'strlen')), $pd);
                    }
                    elseif ($tf == $ff)
                    {
...
as$nf => $Wg)
                    {
                        if (preg_match('~^(.+)(\(.*\))?$~U', $nf, $A))
                        {
                            grant("REVOKE", array_keys($Wg), $A[2], " ON $A[1] FROM $ff");
                        }
                    }
                }
            }
...
as$nf => $pd)
            {
                ...
                $Y = $pd[strtoupper($tg)];
                if ($Db == "Server Admin" && $nf != (isset($qd["*.*"]) ? "*.*" : ".*"))
                {
                    echo"<td>";
                }
                elseif (isset($_GET["grant"]))
                {
                    echo"<td><select name=$B><option><option value='1'" . ($Y ? " selected" : "") . ">" . lang(243) . "<option value='0'" . ($Y == "0" ? " selected" : "") . ">" . lang(244) . "</select>";
                }
                else
                {
...
as$y => $X)
        {
            echo"<td>" . (($x == "sql" && $y == "Info" && preg_match("~Query|Killed~", $I["Command"]) && $X != "") || ($x == "pgsql" && $y == "current_query" && $X != "<IDLE>") || ($x == "oracle" && $y == "sql_text" && $X != "") ? "<code class='jush-$x'>" . shorten_utf8($X, 100, "</code>") . ' <a href="' . h(ME . ($I["db"] != "" ? "db=" . urlencode($I["db"]) . "&" : "") . "sql=" . urlencode($X)) . '">' . lang(246) . '</a>' : h($X));
        }
        echo"\n";
    }
    echo'</table>
...
as$B => $X)
                ...
                if ($_POST["delete"] || $N)
                {
                    if ($_POST["clone"])
                    {
                        $F = "INTO " . table($a) . " (" . implode(", ", array_keys($N)) . ")\nSELECT " . implode(", ", $N) . "\nFROM " . table($a);
                    }
...
as$y => $X)
            ...
            else
            {
                ...
                foreach ($Ie[0]as$y => $X)
                {
                    ...
                    {
                        $N = [];
                        foreach ($Je[1]as$s => $ob)
                        {
                            $N[idf_escape($sb[$s])] = ($ob == "" && $p[$sb[$s]]["null"] ? "NULL" : q(str_replace('""', '"', preg_replace('~^"|"$~', '', $ob))));
                        }
                        $J[] = $N;
                    }
                }
    ...
    if (isset($Xg["insert"]) || !support("table"))
    {
        ...
        {
            if ($id[$X["col"]] && count($id[$X["col"]]) == 1 && ($X["op"] == "=" || (!$X["op"] && !preg_match('~[_%]~', $X["val"]))))
            {
...
as$y => $X)
        ...
        else
        {
            ...
            if ($_GET["page"] != "last" && $z != "" && $rd && $ce && $x == "sql")
            {
                $ld = $h->result(" SELECT FOUND_ROWS()");
            }
            if (!$J)
            {
...
apply_sql_function($X["fun"], $B) . "</a>";
                            echo"<span class='column hidden'>","<a href='" . h($Fd . $bc) . "' title='" . lang(60) . "' class='text'> â†“</a>";
                            if (!$X["fun"])
                            {
...
as$y => $X)
                        {
                            $ze[$y] = max($ze[$y], min(40, strlen(utf8_decode($X))));
                        }
                    }
                }
...
as$y => $X)
                    {
                        if (isset($cf[$y]))
                        {
                            ...
                            {
                                $_ = ME . 'download=' . urlencode($a) . '&field=' . urlencode($y) . $Ki;
                            }
                            if (!$_ && $X !== NULL)
                            {
                                foreach ((array)$id[$y]as$q)
                                {
                                    ...
                                    {
                                        $_ = "";
                                        foreach ($q["source"]as$s => $Ah)
                                        {
                                            $_ .= where_link($s, $q["target"][$s], $J[$bf][$Ah]);
                                        }
            ...
            if (!is_ajax())
            {
                if ($J || $D)
                {
                    ...
                    if ($_GET["page"] != "last")
                    {
                        ...
                        elseif ($x != "sql" || !$ce)
                        {
                            ...
                            if ($ld < max(1e4, 2 * ($D + 1) * $z))
                            {
                                $ld = reset(slow_query(count_rows($a, $Z, $ce, $rd)));
                            }
                            else
                            {
                                $Fc = FALSE;
                    ...
                    if ($Rf)
                    {
                        echo(($ld === FALSE ? count($J) + 1 : $ld - $D * $z) > $z ? '<p><a href="' . h(remove_from_uri("page") . "&page=" . ($D + 1)) . '" class="loadmore">' . lang(258) . '</a>' . script("qsl('a').onclick = partial(selectLoadMore, " . (+$z) . ", '" . lang(259) . "â€¦');", "") : ''),"\n";
                    }
                }
                ...
                if ($J || $D)
                {
                    if ($Rf)
                    {
                        ...
                        if ($x != "simpledb")
                        {
                            echo"<legend><a href='" . h(remove_from_uri("page")) . "'>" . lang(260) . "</a></legend>",script("qsl('a').onclick = function () { pageClick(this.href, +prompt('" . lang(260) . "', '" . ($D + 1) . "')); return false; };"),pagination(0, $D) . ($D > 5 ? " â€¦" : "");
                            for ($s = max(1, $D - 4);$s < min($Le, $D + 5);$s++)
                            {
                                echo
pagination($s, $D);
                            }
                            if ($Le > 0)
                            {
                                echo($D + 5 < $Le ? " â€¦" : ""),($Fc && $ld !== FALSE ? pagination($Le, $D) : " <a href='" . h(remove_from_uri("page") . "&page=last") . "' title='~$Le'>" . lang(261) . "</a>");
                            }
                        }
                        else
                        {
                            echo"<legend>" . lang(260) . "</legend>",pagination(0, $D) . ($D > 1 ? " â€¦" : ""),($D ? pagination($D, $D) : ""),($Le > $D ? pagination($D + 1, $D) . ($Le > $D + 1 ? " â€¦" : "") : "");
                        }
                        echo"</fieldset>\n";
                    }
            ...
            if (!is_view($R))
            {
                ...
                foreach ($Rh + ["Auto_increment" => 0,"Rows" => 0]as$y => $X)
                {
                    if ($R[$y] != "")
                    {
                        ...
                        if (isset($Rh[$y]))
                        {
                            $Rh[$y] += ($R["Engine"] != "InnoDB" || $y != "Data_free" ? $R[$y] : 0);
                        }
                    }
...
else
{
    ...
    if ($ai && !$n && !$_POST["search"])
    {
        ...
        elseif ($G = queries(($_POST["optimize"] ? "OPTIMIZE" : ($_POST["check"] ? "CHECK" : ($_POST["repair"] ? "REPAIR" : "ANALYZE"))) . " TABLE " . implode(", ", array_map('idf_escape', $_POST["tables"]))))
        {
            while ($I = $G->fetch_assoc())
            {
                $Qe .= "<b>" . h($I["Table"]) . "</b>: " . h($I["Msg_text"]) . "<br>";
            }
        }
