...
namespace UTILS
{
    ...
    {
        return ['Core', 'curl', 'date', 'dom', 'fileinfo', 'filter', 'gd', 'gettext', 'hash', 'iconv', 'intl', 'json', 'mbstring', 'libxml','mysqli', 'pcre', 'session', 'SimpleXML', 'SPL', 'xmlreader', 'zip'];
    }
    /**
    ...
    */
     ...
     */
    function writeUserPassword($dbo, $usersId, $usersPassword)
    {
     ...
     *
     * Unfortunately the hashes used with crypt have not been memorized
     * and it is not possible to do the double hash step.
     *
     * So the option chosen is to hash the password the next time a user connects
     * and in a future version remove the old algorithm and reset the password
     * for users who have never reconnected.
     *
     ...
     * @param object $dbo An SQL object
     * @param string $usersId A user Id
     * @param string $usersPassword A clear password to verify
     *
     * @return bool
     */
    function verifyUserPassword($dbo, $usersId, $usersPassword)
    {
        ...
        if ($recordset !== FALSE)
        {
            ...
            $t = explode('$', $row["usersPassword"]);
            // With password_hash, hashes uses format "$..$....$......"
            if (count($t) === 4)
            {
                if (\password_verify($usersPassword, $row["usersPassword"]))
                {
                    if (\password_needs_rehash($row["usersPassword"], PASSWORD_DEFAULT) === TRUE)
                    {
                        writeUserPassword($dbo, $usersId, $usersPassword);
                    }
                    return TRUE;
                }
                else
            ...
            {
                if (\crypt($usersPassword, $row["usersPassword"]) == $row["usersPassword"])
                {
                    writeUserPassword($dbo, $usersId, $usersPassword);
                    return TRUE;
                }
