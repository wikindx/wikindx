...
class RTFEXPORT
{
    ...
    private function getCiteDetails($rowT)
    {
        ...
        $paragraph = $rowT['resourcemetadataParagraph'] ?
                $this->messages->text("resources", "paragraph") . ' ' . $rowT['resourcemetadataParagraph'] : FALSE;
        if ($paragraph)
        {
            $citeArray[] = "$paragraph";
        }
        $chapter = $rowT['resourcemetadataChapter'] ?
                $this->messages->text("resources", "chapter") . ' ' . stripslashes($rowT['resourcemetadataChapter']) : FALSE;
        if ($chapter)
        {
            $citeArray[] = "$chapter";
        }
        $section = $rowT['resourcemetadataSection'] ?
                $this->messages->text("resources", "section") . ' ' . stripslashes($rowT['resourcemetadataSection']) : FALSE;
        if ($section)
        {
            $citeArray[] = "$section";
        }
