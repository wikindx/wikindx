...
<li>Document searches can be performed on resource attachments if they are of type PDF, DOC, DOCX, or RTF – searches are carried out on the cached versions of attachments. If you use the first select field to search on attachments any 'NOT' in the search field will be ignored. Attachment searches are not filtered for the list of ignored words (see below)</li>
...
. dgettext($domain, "<p>Wikindx is not able to renew LDAP password on expiration or bulk import users from a group or OU. It only checks for authentication.</p>")
. dgettext($domain, "<p>When LDAP authentication is enabled, native authentication is not used except for an attempt to connect the Super Admin. If its LDAP authentication fails, the built-in authentication is used instead.</p>")
. dgettext($domain, "<p>When LDAP authentication is successful, the user is created in Wikindx if it did not already exist. The Username (login), Fullname and Email attributes are acquired from the directory server.</p>")
. dgettext($domain, "<p>Passwords are not stored in Wikindx database when performing an LDAP authentication. If the LDAP server is offline or any error during the communication with the server occurs, the LDAP authentication will fail.</p>")
...
. dgettext($domain, "<p>The user MUST enter his login WITHOUT domain information. When you have specified the domain name and the 'user' or 'binduser'  binding methods (connection to the server) are set, the domain name will be added automatically to the login according to the configuration.</p>")
. dgettext($domain, "<p>First, the binding is configured with the chosen protocol. If the binding is encrypted, the certificates will be ignored. Referrals are always ignored. The network connection timeout is set to 10 s. The LDAP search timeout is set to 15 s. The server is contacted and if the connection is successful then WIKINDX searches for the user and verifies its password.</p>")
. dgettext($domain, "<p>The user is found in five steps:</p>")
. dgettext($domain, "<ol>")
. dgettext($domain, "<li>The DN of all user-type objects (SAM_NORMAL_USER_ACCOUNT: sAMAccountType = 805306368) whose login attribute corresponds to the input, which are in one of the configured OUs, is extracted in a first list.</li>")
. dgettext($domain, "<li>The DN of all member objects of group-type objects configured (SAM_GROUP_OBJECT: sAMAccountType = 268435456 or SAM_NON_SECURITY_GROUP_OBJECT: sAMAccountType = 268435457) is extracted in a second list.</li>")
. dgettext($domain, "<li><ol><li>If the search by OU has been configured but not the search by group, the DNs of the first list are kept.</li><li>If the search by group has been configured but not the search by OU, the DNs in the second list are kept.</li><li>If both searches are configured and 'AND' operator is set, the entries common to both lists are kept.</li><li>If both searches are configured and 'OR' operator is set, the entries of both lists are kept.</li><li>If no search has been configured, authentication fails.</li></ol></li>")
. dgettext($domain, "<li>The remaining entries are filtered to retain only the user objects whose login is the one entered.</li>")
. dgettext($domain, "<li>Finally the password of each remaining DN is verified. The first one with a matching password will be authenticated.</li>")
. dgettext($domain, "</ol>")
. dgettext($domain, "<p>A user should not have more than one DN but in the unlikely event that he has two with the same password only the first of the DNs in alphabetical order (regardless of locale) will be authenticated.</p>"),
...
. dgettext($domain, "<ul>")
. dgettext($domain, "<li>Language/Locale: The default interface language of WIKINDX is English (United Kingdom). Translations are included in the kernel and updated with new versions as the translators work. Even if a language is not installed, the user can still set the environment locale (for the display of numbers, dates, currencies etc.). In all cases, missing messages in the selected language will be replaced by the default English. If the language option is set to 'Auto,' the set language will be determined by the browser's locale.</li>")
. dgettext($domain, "<li>Paging links: If a resource list operation returns a high number of resources, this can take time to display. You can, however, define a subset of resources to display in the web browser then use paging links to move onto the next subset.</li>")
. dgettext($domain, "<li>Tag clouds: Resources can be browsed (under the Search menu) as 'tag clouds' where the size and colour of the font represent relative frequency of the data being browsed.</li>")
. dgettext($domain, "<li>Character limiting: Sometimes the text displayed in a form select box is long enough to cause problems with the web browser display. This is often the case with journal titles or publisher names. This field allows the user to limit the amount of text with any dropped text being replaced by ' ... '.</li>")
...
. dgettext($domain, "<li>BibTeX:  BibTeX is a bibliographic format familiar to engineers and scientists.</li>")
. dgettext($domain, "<li>CMS: If the administrator allows it, bibliographic data from WIKINDX may be displayed in Content Management Systems such as MediaWiki, Moodle and WordPress. To aid in this, you can display a 'cms' link that provides the data required.</li>")
...
. dgettext($domain, "<li>User bibliographies: These are drawn from the WIKINDX Master Bibliography and may be personal or managed by a user group. Operations on a user bibliography (such as removing resources from it) have no effect on the WIKINDX Master Bibliography.</li>")
