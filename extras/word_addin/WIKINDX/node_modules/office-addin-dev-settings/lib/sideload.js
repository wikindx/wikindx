        ...
        switch (appType) {
            case appType_1.AppType.Desktop:
                if (!isSideloadingSupportedForDesktopHost(app)) {
                    throw new Error(`Sideload to the ${office_addin_manifest_1.getOfficeAppName(app)} app is not supported.`);
                }
                yield dev_settings_1.registerAddIn(manifestPath);
                // for Outlook, open Outlook.exe; for other Office apps, open the document
                sideloadFile = (app === office_addin_manifest_1.OfficeApp.Outlook)
                    ? yield getOutlookExePath()
                    : yield generateSideloadFile(app, manifest, document);
                break;
            case appType_1.AppType.Web:
                ...
                if (!isSideloadingSupportedForWebHost(app)) {
                    throw new Error(`Sideload to the ${office_addin_manifest_1.getOfficeAppName(app)} web app is not supported.`);
                }
