 ...
 *
 * No parameters are provided, and the return value should be a string that is a valid session ID for your handler.
 *
 * @param string $sessionId
 *
 * @return string
 */
function wkx_session_create_sid(string $sessionId) : string
{
    return "";
}
 ...
 *
 * The key is the session ID to validate. A session ID is valid, if a session with that ID already exists.
 ...
 * @return bool
 */
function wkx_session_validate_sid(string $sessionId) : bool
{
    $db = FACTORY_DB::getInstance();
    
    $db->formatConditions(["sessionId" => $sessionId]);
    $sessionIdDb = $db->selectFirstField("session", "sessionId");
    
    return ($sessionId === $sessionIdDb);
}

/*
 * This callback is executed when a session is updated. key is the session ID, val is the session data.
 *
 * The return value should be true for success, false for failure. 
 *
 * @param string $sessionId
 ...
 * @return bool
 */
function wkx_session_update_timestamp(string $sessionId, string $sessionData) : bool
{
    $db = FACTORY_DB::getInstance();
